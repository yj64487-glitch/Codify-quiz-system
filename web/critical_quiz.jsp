...
<%
    ...
    try {
        ...
        // Fetch 10 Critical Questions
        // Note: Using 'mcq_questions' table (category='mind', level='critical')
        String sql = "SELECT * FROM mcq_questions WHERE category='mind' AND level='critical' AND status='Active' ORDER BY RAND() LIMIT 10";
...
<head>
<meta charset="UTF-8">
<title>Mind Matrix | Critical</title>
<meta name="viewport" content="width=device-width, initial-scale=1.0">
<style>
/* CRITICAL THEME (Blue/Cyan - Same as Foundation) */
*{ margin:0; padding:0; box-sizing:border-box; font-family:Inter,sans-serif; }
body{ min-height:100vh; display:flex; justify-content:center; align-items:center; background: radial-gradient(circle at top,#38bdf833,transparent 40%), #020617; color:#e5e7eb; }
.hidden{display:none}
.quiz-container{ width:92%; max-width:720px; background:#020617; border-radius:26px; padding:34px; box-shadow:0 0 55px rgba(56,189,248,0.35); animation:fadeUp .6s ease; }
@keyframes fadeUp{ from{opacity:0;transform:translateY(30px)} to{opacity:1;transform:translateY(0)} }
.quiz-header{ display:flex; justify-content:space-between; align-items:center; margin-bottom:24px; }
.quiz-title{ font-size:26px; font-weight:900; color:#38bdf8; } 
...
.question-box{ background:#0f172a; padding:26px; border-radius:18px; font-size:19px; margin-bottom:24px; border-left:4px solid #38bdf8; }
...
.options label:hover{ border-color:#38bdf8; transform:translateX(6px); }
.options input{margin-right:12px}
.nav-buttons{ display:flex; justify-content:space-between; margin-top:30px; }
.nav-buttons button{ padding:14px 30px; border:none; border-radius:14px; background:linear-gradient(135deg,#38bdf8,#0ea5e9); color:#020617; font-weight:900; cursor:pointer; }
.nav-buttons button:disabled{ background:#334155; cursor:not-allowed; }
.view-result{ text-align:center; margin-top:28px; }
.view-result button{ padding:16px 44px; border:none; border-radius:999px; background:linear-gradient(135deg,#22c55e,#16a34a); color:#020617; font-weight:900; font-size:16px; cursor:pointer; }
.result-container{ width:92%; max-width:560px; background:#020617; border-radius:26px; padding:38px; text-align:center; box-shadow:0 0 70px rgba(56,189,248,0.45); animation:fadeUp .6s ease; }
...
.result-actions button{ padding:14px 28px; border:none; border-radius:14px; background:linear-gradient(135deg,#38bdf8,#0ea5e9); color:#020617; font-weight:900; cursor:pointer; margin: 10px; }
</style>
</head>
<body>
...
<div class="quiz-container" id="quizBox">
  <div class="quiz-header">
    <div class="quiz-title">Mind Matrix – CRITICAL</div>
...
<div class="result-container hidden" id="resultBox">
  <h2 style="color:#38bdf8">Quiz Result</h2>
...
function load(){
  if(questions.length===0){ qBox.innerText="No Critical Questions Found."; return; }
  ...
  questions[index].options.forEach(function(opt){
    ...
    lbl.onclick=function(){ 
        ...
        lbl.style.borderColor="#38bdf8";
        lbl.querySelector("input").checked = true;
    };
    
    if(isChecked) lbl.style.borderColor="#38bdf8";
...
function finishQuiz(){
  ...
  // SAVE
  // fetch("save_score.jsp?type=critical&score="+score, {method:'POST'});
